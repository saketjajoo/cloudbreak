...
public class MockPatchService extends ExistingStackPatchService {
    ...
    static final String STACK_PATCH_TRIES_TAG = "stack-patch-tries";
    ...
    private void incrementTriesTag(Stack stack, StackTags stackTags) {
        int tryCount = Optional.ofNullable(stackTags.getUserDefinedTags().get(STACK_PATCH_TRIES_TAG))
                .map(Integer::parseInt)
                .orElse(0);
        stackTags.getUserDefinedTags().put(STACK_PATCH_TRIES_TAG, String.valueOf(tryCount + 1));
